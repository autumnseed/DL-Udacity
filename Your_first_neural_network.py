...
print(MSE(network.run(test_features).T,test_targets['cnt'].values))
...
# test data的损=0.33
# 
# 预测效果欠佳，12月前部分underfitting, 后部分overfitting，因为数据本身带时序，所以应该考虑节假日的影响，例如12月底，节假日比较多，尤其是25左右，可能放假在家都不会使用共享单车所以使用量会少很多，这个应该把节假日考虑在内
